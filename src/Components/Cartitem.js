...
function Cartitem({cart}){
    ...
    const handlecountmax=(cartindex)=>{
            const updcart=newCart.map((item,index)=>{
                return cartindex===index?{ ...item,quantity : item.quantity+1}:item
            })
            setNewCart(updcart)
    }

    const handlecountmin=(cartindex)=>{
            const updcart=newCart.map((item,index)=>{
                return cartindex===index? item.quantity!==0?{ ...item,quantity : item.quantity-1}:item :item
            })
            setNewCart(updcart)
    }

    let count=newCart.length
...
<div className="row justify-content-center">
        <div class="col-md-4 mb-4 mx-3 position-static">
          ...
          {newCart?.map((element,cartindex)=>{
            ...
            <div class="row">
              ...
              <div class="col-md-6 ms-3">
                ...
                <p class="text-descriptions mt-0">Qty:<span class="text-descriptions fw-bold"><button className="btn btn-link px-2" onClick={()=>handlecountmin(cartindex)}>
                        <i class="fas fa-minus"></i>
                        </button>{element.quantity}<button className="btn btn-link px-2" onClick={()=>handlecountmax(cartindex)}>
            ...
            <div class="card-footer mt-4">
              <ul class="list-group list-group-flush">
                <li
                  ...
                  Subtotal
                  <span>{
                newCart.map(element=>element.price * element.quantity).reduce((total,value)=> total + value, 0)
            }</span>
                </li>
                <li
